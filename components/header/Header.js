...
const Header = ({activeNavItem})=>{
    ...
    return(
       <nav className="fixed z-40 bg-[var(--color-pray)]  lg:bg-transparent  w-full lg:w-[5rem] px-2 py-4 bottom-0 right-0 lg:top-[22vh] h-[5rem] lg:h-[max-content] lg:right-5 text-[var(--color-text)] ">
        <article className="w-[100%] flex lg:flex-col justify-around lg:justify-center ">
         ...
         <Link className={`animate__animated ${portfolioActive?"bg-[var(--color-orange)] animate__shakeY":""} group flex justify-center transition-all duration-500 ease-in-out cursor-pointer items-center p-3 relative bg-[var(--color-gray)] text-slate-100 w-12 h-12 rounded-full lg:my-2 hover:bg-[var(--color-orange)]`} href="/Portfolio" title="نمونه کارها"><h2 className="hidden cursor-pointer transition-all duration-500 ease-in-out lg:inline-block bg-[var(--color-orange)] lg:opacity-0  lg:absolute w-0 right-0 p-3 rounded-full lg:group-hover:opacity-100 lg:group-hover:w-32 text-center text-lg overflow-hidden max-h-12">نمونه کارها</h2><i className='bx bxs-briefcase text-2xl'></i></Link>
