    ...
    return(
       <nav className="fixed z-50 bg-[var(--color-pray)]  lg:bg-transparent  w-[100%] lg:w-[5rem] px-2 py-4 bottom-0 lg:top-[22vh] h-[5rem] lg:h-[max-content] lg:right-5 text-[var(--color-text)] ">
        <article className=" flex lg:flex-col justify-around lg:justify-center w-[100%]">
         ...
         <Link className={`animate__animated ${blogActive?"active animate__shakeY":""} group flex justify-center transition-all duration-500 ease-in-out cursor-pointer items-center p-3 relative bg-[var(--color-gray)] text-slate-100 w-12 h-12 rounded-full lg:my-2 hover:bg-[var(--color-orange)]`} href="/Blog" title="وبلاگ"><h2 className="hidden cursor-pointer transition-all duration-500 ease-in-out lg:inline-block bg-[var(--color-orange)] lg:opacity-0  lg:absolute w-0 right-0 p-3 rounded-full lg:group-hover:opacity-100 lg:group-hover:w-32 text-center text-lg overflow-hidden max-h-12">وبلاک</h2><i className='bx bxs-conversation text-2xl'></i></Link>
        </article>
       </nav>
    )
}
