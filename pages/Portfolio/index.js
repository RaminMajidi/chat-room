...
const Portfolio = (props)=>{
    const[catregorys,setCategorys] = useState()
...
filterCategory == "All" ? (setFilterData(data)) :(setFilterData(data.filter(item=>item.category === filterCategory)))
    ...
    return(
        ...
        {catregorys &&
            catregorys.map((item,index)=>(
            <button onClick={()=>filterHandeler(item)} key={index} className={`${item == filter ? "text-[var(--color-orange)]" : "text-[var(--color-text)]"} text-xl  font-[Poppins] tracking-wide my-2 mx-3 p-1`}>{item}</button>
            )) 
        }
        </div>
        <div className=" w-full flex flex-wrap justify-center max-w-7xl mx-auto my-2 px-2 py-4">
            {filterData.length ? (
             ...
             (
                <div className="animate__animated animate__zoomIn">
                    <Image width={400} height={200} src={"/images/not-item.png"} />
                    <p className="text-[var(--color-orange)] text-2xl font-bold  text-center">متاسفم آیتمی یافت نشد !!!</p>
                </div>
             )}
        </div>
        </Layout>
